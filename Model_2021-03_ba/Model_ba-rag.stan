functions {
  ...
  matrix simulate(vector initialstate, int time_max,
    ...
    for (t in 2:time_max) {
      ...
      State[i_j, t]  =  log1p(expm1(J_log) + r) - c_j*sum(J) - s*BA - m_j - g; // equivalent to … =  J + (r - (c_j*sum(J) + s*AB + g + m_j).*J * dt;
      State[i_a, t]  =  log1p(A + expm1(J_log - g)) - c_a*BA - m_a - h;
      State[i_b, t]  =  log1p(B + expm1(A_log - h) * ba_a_upper) + b - c_b*sum(B); // exp(A_log - h) * ba_a_upper is the input of new basal area through ingrowth of count A*exp(-h) from A
    ...
    for (l in 1:N_locs) {
      ...
      matrix[n_p, time_max[l]] States =
                        simulate(segment(state_init, pos_pops, n_p), // segment has length n_pops. Structure state_init: locs/stage/species 
                                 ...
                                 b[s_species], c_j[s_species], c_a[s_species], c_b[s_species], h[s_species], m_a[s_species],  // environmentally-independent parameters have been transformed through lognormal
                                 ba_a_avg, ba_a_upper,
                                 n_s, n_p,
                                 i_j, i_a, i_b);
...
model {
  ...
  vector[N_yhat] y_hat_log = unpack(state_init_log, time_max, times, species, pops,
                                    ...
                                    b, c_j, c_a, c_b, h, m_a, // rates vector[N_totalspecies]
                                    ba_a_avg, ba_a_upper,
