...
transformed data {
  ...
  int N_fix = 24; // with proportional: 42; // an array of vectors[N_species] { J, A, B, BA, eps, n_iter, 2 * 1 (or 2) * 9 diff_ko_parameter }
  ...
  if (generateposteriorq) {
    ...
    for(loc in 1:N_locs) {
      ...
      if (converged_fix[loc]) { // && convergent[loc]
        ...
        major_fix_switch_l_r[loc] = ba_fix_switch_l_r[loc, 1] > ba_fix_switch_l[loc, 2]; // ba_1 > 50%
