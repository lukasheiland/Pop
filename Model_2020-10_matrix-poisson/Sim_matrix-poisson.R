...
S <- simulateMultipleSeries(format = "long", n_times = 30)
...
getInits <- function() {
  list(
    ...
    state_init =  abs(rnorm(data$y_init, data$y_init, 0.3)) # t(replicate(data$N_series, runif(data$N_pops, 10, 20), simplify = "array")), # stan expects vector<lower=0>[N_species] state_init[N_series]
  )
}
...
getInitsTrue <- function() {
  list(
    ...
    state_init =  abs(rnorm(data$y_init, data$y_init * 1.5)) # stan expects vector<lower=0>[N_species] state_init[N_series]
  )
}


sims <- simulateMultipleSeries(format = "list")
...
Compare <- cbind(estimate = Estimates[c(62:103, 32:61),],
                 true = c(c(A), f, c(data$y_init)))
View(Compare)
