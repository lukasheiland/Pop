...
data {
  ...
  int<lower=0> y_init[N_series, N_pops];
  int<lower=0> Y[N_series, N_obs, N_pops]; // two-dimensional array of vectors[N_pops]
}
...
model {
  ...
  // Model
  for(z in 1:N_series) {
    y_init[z, ] ~ poisson(state_init[z]); // Separately fitting initial state to feed to integrator.

    for (t in 1:N_obs) {
      Y[z,t, ] ~ poisson(State[z,t]); // Lognormal model of states.
    }
