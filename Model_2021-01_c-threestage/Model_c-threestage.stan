functions {
  ...
  vector simulate(vector state_init, real[] time_init, real[] times, int[] species,
                  matrix r, matrix s, matrix g, // env-dependent
    ...
    for (l in 1:N_locs) {
      ...
      // Returns an array of vectors, accessed via State[i]
      ...
      vector[n_p] States[n_t] =
        ode_rk45_tol(ds_dt,
                     ...
                     exp(r[l, s_species]'), exp(s[l, s_species]'), exp(g[l, s_species]'), // link function: r, l, g are still on the log scale
...
data {
  ...
  real times[N_times]; // locations/resurveys
...
transformed parameters {
  ...
  vector[N_yhat] y_hat = simulate(state_init, time_init, times, species,
                                  r_log, s_log, g_log, // rates matrix[N_locs, N_totalspecies]; will have to be transformed
...
model {
  ...
  sigma_par ~ normal(0, 2); // for time plot (plots) from locations; Gaussian has the least heavy tails, even less than expontial.
  ...
  //---------- MODEL ---------------------------------
  // debugging: print("sigma_par ", sigma_par); print("r_loc ", r_loc[l,]);
  
  //// Level 1 (species) to 2 (locs). Transition rate from A to B, directly, without transformation through env, from level 1 to 2.
  for (l in 1:N_locs) {
    // r[l,]    ~ lognormal(r_log, sigma_par); // lognormal acts like exp (log-link) on the logmedian
    h[l,]    ~ lognormal(h_log, sigma_par); // lognormal acts like exp (log-link) on the logmedian
    c_j[l,]  ~ lognormal(c_j_log, sigma_par);
    c_ab[l,] ~ lognormal(c_ab_log, sigma_par);
  }
